import Image from "next/image"
import { Title } from "../ui/atoms/title"
import { Paragraph } from "../ui/atoms/paragraph"
import { Input } from "../ui/input"
import { Button } from "../ui/button"

export const HeroSection = () => {
  return (
    <section className="bg-white dark:bg-gray-900">
      <div className="grid max-w-screen-xl px-4 py-8 mx-auto lg:gap-8 xl:gap-0 lg:py-16 lg:grid-cols-12">
        <div className="mr-5 place-self-center lg:col-span-7">
          <Title>Bienvenido a la Comuna Bambil Collao</Title>
          <Paragraph className="lg:mb-8 md:text-lg lg:text-xl">
            Un sitio web para la comunidad de Bambil Collao, donde podrás encontrar información relevante sobre la comuna, noticias, eventos y mucho más.
          </Paragraph>
          <div className="flex flex-col lg:flex-row lg:items-center lg:space-x-4">

            <Input
              type="email"
              placeholder="Ingresa tu correo electrónico"
              className="w-full"
            />
            <Button className="w-full lg:w-auto mt-4 lg:mt-0">
              Suscribirse
            </Button>
          </div>
        </div>
        <div className="hidden lg:mt-0 lg:col-span-5 lg:flex">
          <Image src="https://ik.imagekit.io/stevenrosales/app-comuna/comuna.webp?updatedAt=1737254562322"
            width={600}
            height={400}
            className="object-cover rounded-lg shadow-lg"
            alt="mockup" />
        </div>
      </div>
    </section>
  )
}
