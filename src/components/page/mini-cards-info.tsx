...
const cardInfo = [
  ...
  },
  {
    key: 'associations',
    label: 'Asociaciones',
    icon: <Landmark className="text-purple-500" aria-label="Icono asociaciones" />,
  ...
  return (
    ...
    >
      {cardInfo.map((info) => (
        <div
          key={info.key}
          className={`flex flex-col items-center justify-center gap-2 rounded-lg bg-white p-4 shadow-md animate-fade-in-card`}
          tabIndex={0}
          aria-label={`${info.label}: ${data[info.key as keyof Data]}`}
        >
          <div className="mb-1" title={info.tooltip}>{info.icon}</div>
          <h2 className={`text-2xl font-semibold ${info.color}`}>{data[info.key as keyof Data]}</h2>
