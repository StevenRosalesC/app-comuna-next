...
export default function DocumentUpload({ memberId, onDocumentUploaded }: DocumentUploadProps) {
  ...
  const uploadMutation = useMutation({
    mutationFn: async ({ file, documentTypeId }: { file: File; documentTypeId: string }) => {
      return memberDocumentsService.uploadDocument(memberId, file, documentTypeId);
    },
    onSuccess: (_, { documentTypeId }) => {
      const isUpdate = isDocumentTypeUploaded(documentTypeId);
      toast.success(isUpdate ? 'Documento actualizado exitosamente' : 'Documento subido exitosamente');
      queryClient.invalidateQueries({ queryKey: ['memberDocuments', memberId] });
  ...
  const downloadMutation = useMutation({
    ...
    onSuccess: (blob, documentTypeId) => {
      const memberDocument = memberDocuments?.find(doc => doc.documentTypeId === documentTypeId);
      const fileName = memberDocument?.originalFileName || `document-${documentTypeId}`;
  ...
  const isDocumentTypeUploaded = useCallback((documentTypeId: string) => {
    return memberDocuments?.some(doc => doc.documentTypeId === documentTypeId) || false;
  }, [memberDocuments]);

  // Get existing document for a type
  const getExistingDocument = useCallback((documentTypeId: string) => {
    return memberDocuments?.find(doc => doc.documentTypeId === documentTypeId);
  }, [memberDocuments]);
  ...
  const handleFileUpload = useCallback((file: File) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      toast.error('Tipo de archivo no permitido. Solo se permiten PDF, imágenes, DOC y DOCX');
      return;
    }
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger>
              <SelectValue placeholder={loadingTypes ? "Cargando tipos..." : "Selecciona un tipo de documento"} />
            </SelectTrigger>
            <SelectContent>
              {availableDocumentTypes.length === 0 ? (
                <SelectItem value="" disabled>
                  {loadingTypes ? "Cargando..." : "No hay tipos de documento disponibles"}
                </SelectItem>
              ) : (
                availableDocumentTypes.map((type) => {
                  const isUploaded = isDocumentTypeUploaded(type.documentTypeId);
                  return (
                    <SelectItem key={type.documentTypeId} value={type.documentTypeId}>
          ...
          {selectedDocumentType && isDocumentTypeUploaded(selectedDocumentType) && (
              ...
              {memberDocuments.map((document) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="text-sm font-medium">{document.documentType.name}</p>
                      <p className="text-xs text-muted-foreground">
                        {document.originalFileName}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        Actualizado: {new Date(document.lastUpdate).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                  <Button
